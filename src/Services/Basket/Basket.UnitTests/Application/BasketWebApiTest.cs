...
public class BasketWebApiTest
{
    ...
    public async Task Get_customer_basket_success()
    {
        ...
        _serviceBusMock.Setup(x => x.Publish(It.IsAny<UserCheckoutAcceptedIntegrationEvent>(), It.IsAny<IDictionary<string, string>>()));
    ...
    public async Task Doing_Checkout_Wit_Basket_Should_Publish_UserCheckoutAccepted_Integration_Event()
    {
        ...
        _serviceBusMock.Verify(mock => mock.Publish(It.IsAny<UserCheckoutAcceptedIntegrationEvent>(), It.IsAny<IDictionary<string, string>>()), Times.Once);
